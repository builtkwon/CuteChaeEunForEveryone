...
# 결과 저장소 — InMemoryResultStore → RedisResultStore 등으로 교체 가능
...
def _make_service(creds_dict: dict) -> PhotoService:
    """요청마다 인증된 사용자의 크리덴셜로 서비스 인스턴스를 생성합니다.
    CloudStorage 구현체 교체는 여기 한 줄만 바꾸면 됩니다.
    """
    creds = dict_to_creds(creds_dict)
    storage = GoogleDriveStorage(creds)   # ← S3Storage(…) 등으로 교체
...
async def upload(
    ...
    service = _make_service(creds_dict)
    file_bytes = await photo.read()
    filename = photo.filename or "photo.jpg"

    file_id = service.process(file_bytes, filename, qr_position)
...
async def preview(file_id: str):
    ...
    return StreamingResponse(io.BytesIO(info["data"]), media_type="image/jpeg")
...
async def download(file_id: str):
    ...
    return StreamingResponse(
        ...
        headers={"Content-Disposition": f'attachment; filename="{stem}_qr.jpg"'},
    )
